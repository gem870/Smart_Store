...
public:
    ...
    /**
     * @brief Draw annotations for detections or tracks.
    ...
    FaceError setScaleFactor(double value);       // default 1.1
    FaceError setMinNeighbors(int value);         // default 3
    FaceError setMinFaceSize(const cv::Size& s);  // default (30,30)
    FaceError setMaxTrackAgeMs(uint64_t ms);      // default 2000
    FaceError setIouMatchThreshold(double t);     // default 0.3
    ...
    bool checkForKnownFaces(const cv::Mat& frame, const FaceTrack& track);
...
private:
    ...
    // Detection params (guarded by mutex_)
